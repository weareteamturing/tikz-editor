TODOS

* Switch cap / join selectors icons to be similar to https://boxy-svg.com/app/

* context menu for path segments: add/remove segments, convert control point to bezier / straight, etc

* Selecting and transforming scopes. Group/ungroup

* Clipboard robustness

* Inspector: allow adding styles to an element with visual style preview

* Open tex document with potentially multiple tikz images; parse relevant macro definitions etc

* Support adding nodes to path

* Add "edge" tool with bend

* Allow adding path segments

* Context menu commands to edit matrices (e.g. add/remove rows/columns)

* Support for editing trees using `child`
* Inspector options for trees: level distance, sibling distance, etc

* Add graphs? e.g. insert standard graphs?

* Edit arc paths

* Add shape tool (i.e. nodes with a shape)

* Take libraries of public domain / open license SVGs (e.g. icon libraries) and treat them as "clip art" style things that can be inserted -- see boxy svg for example of this

* Support for multi-line text in nodes (special difficulty: mathjax select / caret)

* App settings: geometry precision digits, code formatter options

* Any wizards?

* File extension registration for .tikz (primary editor), .tex (secondary editor)

* Updating extension, with .json to live at https://tikz.dev/edit/update.json

* Github Action running on trigger that builds Windows binary

* Dialog "sure you want to close" should be native