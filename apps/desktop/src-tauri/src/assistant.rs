...
struct DocumentAssistantSession {
  ...
  items: Vec<Value>,
  pending_server_requests: HashMap<String, u64>,
...
impl AssistantState {
  ...
  fn start_file_watcher(&self) {
    let mut started = self.inner.watcher_started.lock().expect("watcher lock poisoned");
    if *started {
      return;
    }
    *started = true;
  ...
  fn poll_figure_files(&self) {
    let sessions = {
      let docs = self.inner.documents.lock().expect("documents lock poisoned");
    ...
    for session in sessions {
      ...
      let mut docs = self.inner.documents.lock().expect("documents lock poisoned");
  ...
  fn ensure_process(&self) -> Result<(), String> {
    if self.inner.process.lock().map_err(|_| "process lock unavailable".to_string())?.is_some() {
      return Ok(());
    }
    ...
    {
      let mut process_slot = self.inner.process.lock().map_err(|_| "process lock unavailable".to_string())?;
  ...
  fn request(&self, method: &str, params: Value) -> Result<Value, String> {
    ...
    let (id, stdin, pending) = {
      let process_guard = self.inner.process.lock().map_err(|_| "process lock unavailable".to_string())?;
      let process = process_guard.as_ref().ok_or_else(|| "Codex app-server is unavailable".to_string())?;
      (
    ...
    pending.lock().map_err(|_| "pending lock unavailable".to_string())?.insert(id, sender);
    ...
    {
      let mut writer = stdin.lock().map_err(|_| "stdin lock unavailable".to_string())?;
    ...
    let response = receiver
      ...
      .map_err(|_| format!("Timed out waiting for `{method}` response from Codex App Server"))?;
  ...
  pub fn ensure_document_thread(
    ...
    if let Some(existing) = self.inner.documents.lock().map_err(|_| "documents lock unavailable".to_string())?.get(&document_id).cloned() {
    ...
    self.inner.documents.lock().map_err(|_| "documents lock unavailable".to_string())?.insert(document_id, session.clone());
  ...
  pub fn start_turn(
    ...
    let summary = self.ensure_document_thread(document_id.clone(), source.clone(), None, None, None, None)?;
    ...
    let input = build_turn_input(&summary.figure_path, &summary.preview_path, &prompt, &source);
    let result = self.request(
      "turn/start",
      json!({
    ...
    let turn_id = result
      ...
      .and_then(Value::as_str)
      .map(str::to_string);
    if let Some(session) = self.inner.documents.lock().map_err(|_| "documents lock unavailable".to_string())?.get_mut(&document_id) {
  ...
  pub fn interrupt_turn(&self, document_id: String) -> Result<(), String> {
    ...
    if let Some(turn_id) = session.current_turn_id {
      let _ = self.request("turn/interrupt", json!({ "threadId": session.thread_id, "turnId": turn_id }))?;
    }
    Ok(())
  }

  pub fn sync_source(&self, document_id: String, source: String) -> Result<(), String> {
    let mut docs = self.inner.documents.lock().map_err(|_| "documents lock unavailable".to_string())?;
    let session = docs.get_mut(&document_id).ok_or_else(|| "Assistant thread not initialized for document".to_string())?;
  ...
  pub fn respond_to_approval(&self, document_id: String, request_id: String, decision: String) -> Result<(), String> {
    let server_request_id = {
      let docs = self.inner.documents.lock().map_err(|_| "documents lock unavailable".to_string())?;
      let session = docs.get(&document_id).ok_or_else(|| "Assistant thread not initialized for document".to_string())?;
      session
    ...
    self.send_server_request_response(server_request_id, Value::String(normalize_approval_decision_value(&decision).to_string()))
  }

  pub fn respond_to_dynamic_tool_call(&self, document_id: String, request_id: String, result: Value) -> Result<(), String> {
    if let Some(image_data) = result
      .get("contentItems")
      .and_then(Value::as_array)
      .and_then(|items| items.iter().find(|item| item.get("type").and_then(Value::as_str) == Some("image")))
    ...
    self.send_server_request_response(server_request_id, result)
  }

  pub fn load_thread_state(&self, document_id: String) -> Result<Option<AssistantThreadStatePayload>, String> {
    let docs = self.inner.documents.lock().map_err(|_| "documents lock unavailable".to_string())?;
  ...
  fn send_server_request_response(&self, request_id: u64, result: Value) -> Result<(), String> {
    self.ensure_process()?;
    let stdin = {
      let process_guard = self.inner.process.lock().map_err(|_| "process lock unavailable".to_string())?;
      process_guard
  ...
  fn handle_response(&self, message: Value, pending: &Arc<Mutex<HashMap<u64, Sender<Value>>>>) {
  ...
  fn handle_server_request(&self, message: Value) {
    ...
    if let Ok(mut docs) = self.inner.documents.lock() {
      if let Some(session) = docs.get_mut(&document_id) {
        session.pending_server_requests.insert(id.to_string(), id);
      }
    }
    ...
    match method {
      ...
      "turn/completed" => {
        if let Some(document_id) = document_id {
          ...
          let error = turn
            .get("error")
            .and_then(|value| value.get("message").or_else(|| value.get("error")).cloned())
      ...
      "item/started" | "item/completed" => {
        if let Some(document_id) = document_id {
          ...
          let _ = self.emit_event(AssistantEventPayload {
            kind: if method == "item/started" { "item-started".to_string() } else { "item-completed".to_string() },
            data: json!({
              "documentId": document_id,
              "item": item
            }),
      ...
      "item/agentMessage/delta" | "item/plan/delta" | "item/reasoning/summaryTextDelta" | "item/reasoning/textDelta" | "item/commandExecution/outputDelta" => {
        if let Some(document_id) = document_id {
  ...
  fn upsert_item(&self, document_id: &str, item: &Value) {
    ...
    if let Ok(mut docs) = self.inner.documents.lock() {
      if let Some(session) = docs.get_mut(document_id) {
        if let Some(index) = session.items.iter().position(|existing| existing.get("id").and_then(Value::as_str) == Some(item_id)) {
  ...
  fn emit_event(&self, payload: AssistantEventPayload) -> Result<(), String> {
    self.inner.app.emit(ASSISTANT_EVENT_NAME, payload).map_err(|error| error.to_string())
  }
}
...
fn spawn_stdout_reader(state: AssistantState, stdout: ChildStdout, pending: Arc<Mutex<HashMap<u64, Sender<Value>>>>) {
  thread::spawn(move || {
...
fn resolve_workspace(app: &AppHandle, document_id: &str, provided: Option<&str>) -> Result<PathBuf, String> {
...
fn build_turn_input(figure_path: &str, preview_path: &str, prompt: &str, source: &str) -> Vec<Value> {
