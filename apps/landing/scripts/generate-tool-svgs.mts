...
const OUT_FILE = path.resolve(__dirname, "../src/versions/VersionB/generated/tool-svgs.ts");
...
export const VERSION_B_TOOL_SVGS = ${JSON.stringify(Object.fromEntries(renderedEntries) satisfies Record<string, ToolPreviewOutput>, null, 2)} as const;
`;
...
function renderOverlaySvg(
  ...
  if (selectionBoxes.length > 0) {
    parts.push(`<g class="vBExactSelectionOverlay">`);
  ...
  if (curveControlLines.length > 0) {
    parts.push(`<g class="vBExactCurveControlOverlay">`);
  ...
  if (handleDisplays.length > 0) {
    parts.push(`<g class="vBExactHandleOverlay">`);
