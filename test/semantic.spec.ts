...
\end{tikzpicture}`;
    ...
    expect(result.diagnostics.some((diagnostic) => diagnostic.severity === "error")).toBe(false);
    const gridPaths = result.scene.elements.filter((element) => element.kind === "Path");
...
\end{tikzpicture}`;
    ...
    const opParsed = parseTikz(operationSource);
    const optionParsed = parseTikz(optionSource);
    const opResult = evaluateTikzFigure(opParsed.figure, operationSource);
    const optionResult = evaluateTikzFigure(optionParsed.figure, optionSource);
    const opPath = opResult.scene.elements.find((element) => element.kind === "Path");
    const optionPath = optionResult.scene.elements.find((element) => element.kind === "Path");
...
\end{tikzpicture}`;
    ...
    const rightResult = evaluateTikzFigure(parseTikz(growRight).figure, growRight);
    const reversedResult = evaluateTikzFigure(parseTikz(growRightReversed).figure, growRightReversed);
  ...
  \path node {root} [grow'=up]
    ...
    child { node {right} };
\end{tikzpicture}`;
    const result = evaluateTikzFigure(parseTikz(source).figure, source);
